...
def write_model_cfg(old_path, new_path, new_module_defs):
    ...
    for i, module_def in enumerate(new_module_defs):
        ...
        if mtype == "convolutional":
            ...
            an_rows, an_cols = module_def['anchors'].shape
            tmp_str = ''
            for an_i in range(an_rows):
                tmp_str += '%d,%d, ' % (module_def['anchors'][an_i, 0], module_def['anchors'][an_i, 1])
            print('===============', tmp_str[0:-2])
...
##  output a new cfg file
    ...
    chkpt = {'epoch': -1,
             ...
             'model': _pruned_state_dict,
             'optimizer': None}       
    torch.save(chkpt, prune_weights_path)
